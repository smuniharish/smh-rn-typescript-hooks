...
const useValidateForm = (options) => {
    ...
    const { type, label, isRequired, keyboard, minValue, maxValue, minLength, maxLength, defaultValue, isDisabled, isEditable, validationPattern, minValueError, maxValueError, minLengthError, maxLengthError, focusError, validError, valueChangeCallback, } = options;
    ...
    useEffect(() => {
        if (defaultValue) {
            if (type === 'number' && typeof defaultValue === 'string') {
                throw new Error('Default Value Error: type and defaultValue should be in same data type');
            }
                ...
                multipleAction({
                    inputValue: typeof defaultValue !== 'string'
    ...
    useEffect(() => {
        if (validationPattern) {
            if (!(validationPattern instanceof RegExp)) {
                throw new Error('Given validationPattern is not a valid regex pattern !!!');
            }
        }
    }, [validationPattern]);
    useEffect(() => {
        ...
        if (isRequired) {
            ...
            if (validationPattern) {
                ...
                const numInput = inputValue && type === 'number'
